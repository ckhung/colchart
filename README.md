...
Usage:
```bash
    ./mat2list.pl -r '1:' -c '20:20' freq.csv > rugged.txt
    ./pt2sq.pl -y 20 rugged.txt > colchart.txt
```
Then inside gnuplot:
```gnuplot
    load "colchart.gpt"
...
Usage example:
```./mat2list.pl freq.csv > rugged.txt```
Additionally, the following command line options are available:
...
mat2list.pl) consisting of lines of the format:
...
The pt2sq.pl script does this expansion by taking
...
That is, **pt2sq.pl does not infer the spacing
from the input data**.

Usage example:
```./pt2sq.pl -y 20 rugged.txt > colchart.txt```

Then yuo can ```splot "colchart.txt"``` inside gnuplot
